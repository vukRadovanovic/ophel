...
/// 		- When changing some low level fields that can affect the connectivity of the graph such as <see cref="Pathfinding.MeshNode.connections"/> you must now call <see cref="Pathfinding.GraphNode.SetConnectivityDirty()"/>.
...
/// 		- Added a new <see cref="Pathfinding.GraphMask"/> struct to represent a set of graphs.
/// 			Previously graph masks have been represented using pure integers (e.g. in <see cref="Pathfinding.Seeker.graphMask"/> and <see cref="Pathfinding.NNConstraint.graphMask)"/>.
/// 			The new struct contains some nice helper methods like <see cref="Pathfinding.GraphMask.FromGraphName"/> which has been requested by many users.
/// 		- Added a new documentation page describing the inspector: inspector (view in online documentation for working links).
/// 		- Added a new documentation page with an overview of the different included movement scripts: movementscripts (view in online documentation for working links).
/// 		- Added a new property on all movement scripts called <see cref="Pathfinding.IAstarAI.reachedDestination"/>.
/// 			The existing <see cref="Pathfinding.IAstarAI.reachedEndOfPath"/> property has some quirks, which are very reasonable when considering how everything works,
...
/// 			This in turn deprecates the old <see cref="Pathfinding.AIBase.centerOffset"/> field as it is now implicitly height/2.
...
/// 		- Added <see cref="Pathfinding.AstarData.GetNodes"/>.
...
/// 			For more information about how this works, see <see cref="Pathfinding.HierarchicalGraph"/>.
...
/// 		- Improved performance of get nearest queries on point graphs when not using <see cref="Pathfinding.PointGraph.optimizeForSparseGraph"/>.
...
/// 		- Removed the mostly internal method <see cref="Pathfinding.GraphCollision.Raycast"/> because it was not used by any code in the package.
/// 		- Changed signature and behavior slightly of the mostly internal method <see cref="Pathfinding.GraphCollision.CheckHeightAll"/>.
/// 		- Replaced <see cref="Pathfinding.LayerGridGraph.SampleCell"/> with the new method <see cref="Pathfinding.LayerGridGraph.SampleHeights"/> which does essentially the same thing, but in a more efficient way.
/// 		- The rotationIn2D option for the built-in movement scripts has been renamed to <see cref="Pathfinding.AIBase.orientation"/> and is now an enum.
/// 	- Fixes
...
/// 		- Fixed minor allocation when calling <see cref="AstarPath.GetNearest"/> without an NNConstraint.
...
/// 		- Fixed <see cref="Pathfinding.Path.IsDone"/> could due to a race condition return true a tiny amount of time before the path was actually calculated.
...
/// 		- Fixed <see cref="AstarPath.FlushWorkItems"/> always pausing the pathfinding threads even if no work items were queued.
...
/// 	- Added some video examples to <see cref="Pathfinding.AIPath.pickNextWaypointDist"/>.
...
/// 	- It is now possible to use pathfinding from editor scripts. See editor-mode (view in online documentation for working links).
///
/// - 4.1.13 (2018-03-06)
/// 	- Fixed LayerGridGraph.GetNode not performing out of bounds checks.
/// 	- Exposed a public method <see cref="Pathfinding.PointGraph.ConnectNodes"/> which can be useful if you are creating a graph from scratch using e.g PointGraph.AddNode.
/// 	- Improved the multiple-agent-types (view in online documentation for working links) tutorial.
/// 	- Improved the custom_movement_script (view in online documentation for working links) tutorial, among other things it can now also be followed if you are creating a 2D game.
...
/// 	- Added an <see cref="Pathfinding.AIPath.constrainInsideGraph"/> to AIPath for constraining the agent to be inside the traversable surface of the graph at all times.
...
/// 	- Added tutorial for how to deal with agents of different sizes: multiple-agent-types (view in online documentation for working links).
...
/// 	- Fixed a bug in <see cref="Pathfinding.GraphUtilities.GetContours"/> for grid graphs when the nodes parameter was explicitly passed as non null that could cause some contours not to be generated. Thanks andrewBeers for reporting the bug.
/// 	- Improved documentation for <see cref="Pathfinding.StartEndModifier.Exactness"/>.
///
/// - 4.1.10 (2018-01-21)
...
/// 		- A method called <see cref="Pathfinding.IAstarAI.SetPath"/> has been added to all movement scripts. This replaces some hacks you could achieve by calling the OnPathComplete method on the movement scripts
...
/// 		- Linecast methods will now assign the <see cref="Pathfinding.GraphHitInfo.node"/> field with the last node that was traversed in case no obstacle was hit, previously it was always null.
...
/// 		- Improved performance of querying for the closest node to a point when using the PointGraph and <see cref="Pathfinding.PointGraph.optimizeForSparseGraph"/>.
...
/// 			- The <see cref="Pathfinding.IAstarAI"/> interface can now be used with all movement scripts.
...
/// 				<see cref="Pathfinding.IAstarAI.remainingDistance"/>,
/// 				<see cref="Pathfinding.IAstarAI.reachedEndOfPath"/>,
/// 				<see cref="Pathfinding.IAstarAI.pathPending"/>,
/// 				<see cref="Pathfinding.IAstarAI.steeringTarget"/>,
/// 				<see cref="Pathfinding.IAstarAI.isStopped"/>,
/// 				<see cref="Pathfinding.IAstarAI.destination"/>, and many more.
/// 			- You no longer need to set the destination of an agent using a Transform object, instead you can simply set the <see cref="Pathfinding.IAstarAI.destination"/> property.
...
/// 		- Improved the performance of the <see cref="Pathfinding.RaycastModifier"/> significantly. Common speedups on grid graphs range from 2x to 10x.
/// 		- The RaycastModifier now has a <see cref="Pathfinding.RaycastModifier.Quality quality"/>. The higher quality options use a new algorithm that is about the same performance (or slightly slower) compared to the RaycastModifier in previous versions
...
/// 		- Added <see cref="Pathfinding.GridGraph.CalculateConnectionsForCellAndNeighbours"/>.
...
/// 		- Linecast methods will now assign the <see cref="Pathfinding.GraphHitInfo.node"/> field with the last node that was traversed in case no obstacle was hit.
...
/// 			[Open online documentation to see images]
/// 		- Reduced memory usage of the PointGraph when using <see cref="Pathfinding.PointGraph.optimizeForSparseGraph"/>.
...
/// 		- Added support for 2D physics to the <see cref="Pathfinding.RaycastModifier"/> component.
...
/// 		- Added a 'filter' parameter to <see cref="Pathfinding.PathUtilities.BFS"/> and <see cref="Pathfinding.PathUtilities.GetReachableNodes"/>.
/// 		- Added a method called <see cref="Pathfinding.IAstarAI.SetPath"/> to all movement scripts.
/// 		- Added <see cref="Pathfinding.GraphNode.Graph"/>.
/// 		- Added <see cref="Pathfinding.MeshNode.ContainsPoint(Vector3)"/> in addition to the already existing MeshNode.ContainsPoint(Int3).
/// 		- Added <see cref="Pathfinding.MeshNode.ContainsPointInGraphSpace"/>.
/// 		- Added <see cref="Pathfinding.TriangleMeshNode.GetVerticesInGraphSpace"/>.
/// 		- Added Pathfinding.AstarData.FindGraph(predicate).
/// 		- Added Pathfinding.AstarData.FindGraphWhichInheritsFrom(type).
/// 		- Added a new class <see cref="Pathfinding.GraphUtilities"/> which has some utilities for extracting contours of graphs.
...
/// 	- Documentation
/// 		- Added a documentation page on error messages: error-messages (view in online documentation for working links).
/// 		- Added a tutorial on how to create a wandering AI: wander (view in online documentation for working links).
/// 		- Added tutorial on bitmasks: bitmasks (view in online documentation for working links).
...
/// 			Few people use UnityScript nowadays so that prompt was mostly annoying. UnityScript support can still be enabled, see javscript (view in online documentation for working links).
...
/// 		- The default value for <see cref="Pathfinding.RVOController.lockWhenNotMoving"/> is now false.
...
/// 		- The <see cref="Pathfinding.RVOController.velocity"/> property on the RVOController can now be assigned to and that has the same effect as calling ForceSetVelocity.
...
/// 		- Fixed some cases where circles used for debugging could have a much lower resolution than intended (<see cref="Pathfinding.Util.Draw.Debug.CircleXZ)"/>.
...
/// 		- Fixed a regression in some 4.0.x version where setting <see cref="Pathfinding.RVOController.velocity"/> to make the agent's movement externally controlled
...
/// 		- Fixed a few cases where <see cref="Pathfinding.RichAI"/> would automatically recalculate its path even though <see cref="Pathfinding.RichAI.canSearch"/> was disabled.
...
/// 		- Fixed the MultiTargetPathExample.cs (view in online documentation for working links).
...
/// 		- The DynamicGridObstacle component will now properly update the graph when the object is deactivated since the object just disappeared and shouldn't block the graph anymore.
/// 			Previously it only did this if the object was destroyed, not if it was deactivated.
/// 		- Fixed <see cref="Pathfinding.AILerp"/> ceasing to work properly if one of the paths it tries to calculate fails.
/// 		- Fixed the <see cref="Pathfinding.FunnelModifier"/> could yield a zero length path in some rare circumstances when using custom node links.
...
/// 	- Fixed several bugs relating to <see cref="Pathfinding.NNConstraint.distanceXZ"/> and <see cref="Pathfinding.NavmeshBase.nearestSearchOnlyXZ"/>. Thanks koirat for reporting this.
/// 	- Fixed scanning a graph that threw an error would prevent any future scans. Thanks Baste for reporting this.
/// 	- Added a new get started video tutorial. See getstarted (view in online documentation for working links).
...
///
/// - 4.0.8 (2017-04-28)
/// 	- Added <see cref="Pathfinding.AIBase.rotationIn2D"/> to the AIPath script. It makes it possible to use the Y axis as the forward axis of the character which is useful for 2D games.
...
/// 	- Added a new documentation page and video tutorial (pathfinding-2d) (view in online documentation for working links) showing how to configure pathfinding in 2D games.
///
/// - 4.0.6 (2017-04-21)
...
/// 	- Fixed code typo in the write-modifiers (view in online documentation for working links) tutorial as well as made a few smaller improvements to it.
...
/// 		- Node connections are now represented using an array of structs (of type <see cref="Pathfinding.Connection"/>) instead of
...
/// 				- Agents are now <see cref="Pathfinding.RVO.RVOSimulator.symmetryBreakingBias biased"/> towards passing other agents on the right side, this helps resolve some situations
/// 					with a lot of symmetry much faster.
/// 				- All fuzzy and hard to adjust parameters from the <see cref="Pathfinding.RVO.RVOSimulator"/> component have been removed.
...
/// 				- Added <see cref="Pathfinding.RVO.RVOController.ForceSetVelocity"/> to use when you want agents to avoid a player (or otherwise externally controlled) character.
...
/// 				- Added support for grid graphs to <see cref="Pathfinding.RVO.RVONavmesh"/>.
...
/// 			- Individual graphs can now be scanned using <see cref="AstarPath.Scan(NavGraph)"/> and other related overloads.
...
/// 		- Renamed AIPath.turningSpeed to <see cref="Pathfinding.AIPath.rotationSpeed"/>.
...
/// 	- Documentation
/// 		- Among other things: improved the writing-graph-generators (view in online documentation for working links) guide (among other things it no longer uses hard to understand calculations to get the index of each node).
///
/// - 3.8.8.1 (2017-01-12)
/// 	- Fixes
...
/// 			- accessing-data (view in online documentation for working links)
/// 			- graph-updates (view in online documentation for working links)
/// 			- writing-graph-generators (view in online documentation for working links)
...
/// 		- Fixed NullReferenceException which could occur sometimes when using a MultiTargetPath (Issue <see cref="16)"/>
...
/// 	Note: Some features are restricted to Unity 3.5 and newer because of technical limitations in earlier versions (especially multi-object editing related features).
///
...
/// 	- There is now only one tag per node (if not the \<see cref="define"/> ConfigureTagsAsMultiple is set).
...
/// 	- Note: Dropped Unity 3.3 support.
///
...
/// 	- Using \<see cref="defines"/>, the pathfinder can now be better profiled (see Optimizations tab -> Profile Astar)
